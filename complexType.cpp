...
//overload the operator ! (finds absolute value)
double complexType::operator!()
{
    realPart = realPart * realPart;
    imaginaryPart = imaginaryPart * imaginaryPart;
    
    double addPart; //realPart^2 + imaginaryPart^2

    addPart = realPart + imaginaryPart;

    double absPart; //absolute value

    absPart = sqrt(addPart); //find absolute value

    return absPart;

}

//overload operator ~ (finds conjugate)
complexType complexType::operator~()
{
    return complexType(realPart, -1 * imaginaryPart);
}